...
object Third_Party_E_Commerce {
  def main(args: Array[String]): Unit = {
    ...
    my_log("Get commerce data of the third party")
    val third_party_ecommerce_table = "algo.up_yf_shopping_interest_3d_ecommerce"
    val notification_data = get_3d_ecommerce_data(sparkSession, third_party_ecommerce_table, job_date)
  }

  def get_3d_ecommerce_data(sparkSession: SparkSession, table_name: String, job_date: String) = {
    ...
    val select_sql = "select imei, title, source from " + table_name + " where stat_date=" + job_date + " and source in (" + source_str + ")"
    print_sql(select_sql)

    val data = sparkSession.sql(select_sql).filter("imei is not null and title is not null and source is not null").rdd.map(v => (v(0).toString, v(1).toString, v(2).toString)).filter(v => v._2.length > 0)
    val data_count = data.count()
    printf("\n====>>>> third party e-commerce data: %d\n", data_count)

    data.map(v => (v._3, 1)).reduceByKey(_+_).collect().foreach(v => {
