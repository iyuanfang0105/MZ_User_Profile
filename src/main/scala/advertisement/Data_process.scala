...
object Data_process {
  def main(args: Array[String]): Unit = {
    ...
    my_log("Get dataset and extract features", level = 1)
    extract_features(sparkSession, job_date)
  }

  def extract_features(sparkSession: SparkSession, job_date: String) = {
    my_log("getting user infos", level = 2)
    val user_infos = get_user_infos(sparkSession)

    my_log("getting adv infos", level = 2)
    val adv_ods_infos = get_advs_info(sparkSession, job_date)

    my_log("getting actions of yesterday", level = 2)
    val yesterday_actions = get_action_yestoday(sparkSession, job_date)

    val historical_actions_offset = -1
    my_log("getting click of user about %d months".format(historical_actions_offset), level = 2)
    val historical_actions = get_historical_actions(sparkSession, historical_actions_offset, job_date)

    // yesterday_actions: imei, unit_id, location, week, hour, time, label
    val data = yesterday_actions.join(user_infos).map(v => (v._2._1._1, (v._1, v._2._1._2, v._2._1._3, v._2._1._4, v._2._1._5, v._2._1._6, v._2._2))).join(adv_ods_infos).map(v => (v._2._1._1, (v._1, v._2._1._2, v._2._1._3, v._2._1._4, v._2._1._5, v._2._1._7._1, v._2._1._7._2, v._2._1._7._3, v._2._1._7._4, v._2._1._7._5, v._2._1._7._6, v._2._1._7._7, v._2._1._7._8, v._2._2._1, v._2._2._2, v._2._2._3, v._2._2._4, v._2._1._6))).leftOuterJoin(historical_actions).map(v => (v._1, v._2._1, v._2._2.getOrElse("null"))).map(v => (v._1, v._2._1, v._2._2, v._2._3, v._2._4, v._2._5, v._2._6, v._2._7, v._2._8, v._2._9, v._2._10, v._2._11, v._2._12, v._2._13, v._2._14, v._2._15, v._2._16, v._2._17, v._3, v._2._18))
    printf("\n====>>>> features: %d\n", data.count())

    import sparkSession.implicits._
    val save_table_name = "algo.up_yf_advertisement_trainning_data_daily_1"
    val cols = "imei string, unit_id string, location string, week string, hour string, date string, is_mz string, sex string, user_age string, user_educational_status string, user_estate string, user_owners string, phone_operator string, phone_model string, idea_slots string, idea_type string, adview_type string, idea_status string, history_click_1m string, label int"
  ...
  def get_action_yestoday(sparkSession: SparkSession, job_date: String) = {
    ...
    val data = sparkSession.sql(select_sql).filter("imei is not null and unit_id is not null and location is not null and posttime is not null and oper_type is not null").rdd.map(v => (v(0).toString, v(1).toString, v(2).toString, v(3).toString, v(4).toString)).distinct().map(v => {
      ...
      if (v._4.length == 19) {
        ...
        week = t._1
        hour = t._2
        time = v._4.substring(5, 10).replaceAll("-", "")
      }
...
//    })
    ...
    data.map(v => (v._1, (v._2, v._3, v._4, v._5, v._6, v._7)))
  }
